from enum import Enum

import hypergraphx
from PyQt5.QtCore import QObject, pyqtSignal

from hypergraphx import Hypergraph, TemporalHypergraph, DirectedHypergraph, MissingNodeError
from hypergraphx.generation import random_hypergraph
from hypergraphx.generation.random import random_uniform_hypergraph
from hypergraphx.readwrite import save_hypergraph
from hypergraphx.utils import connected_components
from hypergraphx.viz.interactive_view.drawing_view.drawing_thread import PlotWorker


class HypergraphType(Enum):
    NORMAL = 1
    DIRECTED = 2
    TEMPORAL = 3


class Controller(QObject):
    """
    Gestisce lo stato dell'ipergrafo e coordina il rendering.

    Segnali
    -------
    updated_hypergraph : dict
        Emesso ogni volta che l'ipergrafo cambia.
        Payload: {"hypergraph": ..., "updated_ccs": bool}
    finished_drawing : DrawingResult
        Emesso quando PlotWorker ha terminato il calcolo del layout.
        Trasporta direttamente il DrawingResult così la vista può
        accedere a tutti i dati in modo tipizzato.
    """

    updated_hypergraph = pyqtSignal(dict)
    finished_drawing = pyqtSignal(object)   # object = DrawingResult

    # ------------------------------------------------------------------
    # Costruttore
    # ------------------------------------------------------------------

    def __init__(self, hypergraph: Hypergraph | TemporalHypergraph | DirectedHypergraph):
        super().__init__()
        self.hypergraph_type: HypergraphType = HypergraphType.NORMAL
        self.showed_hypergraph = None
        self.actual_hypergraph = None
        self.thread: PlotWorker | None = None

        self.change_hypergraph(hypergraph)
        self.cc = connected_components(self.actual_hypergraph)

    # ------------------------------------------------------------------
    # Modifica dell'ipergrafo
    # ------------------------------------------------------------------

    def add_edge(self, values: dict) -> None:
        try:
            if isinstance(self.actual_hypergraph, TemporalHypergraph):
                self.actual_hypergraph.add_edge(
                    values["edge_data"],
                    values["time"],
                    values["weight"],
                    values["metadata"],
                )
            else:
                self.actual_hypergraph.add_edge(
                    values["edge_data"],
                    values["weight"],
                    values["metadata"],
                )
            self.update_hypergraph()
        except Exception:
            raise

    def add_node(self, values: dict) -> None:
        try:
            self.actual_hypergraph.add_node(eval(values["name"]), values["metadata"])
            self.update_hypergraph()
        except Exception:
            raise

    def remove_nodes(self, node_list: list) -> None:
        node_list = list(set(node_list))
        for node in node_list:
            try:
                self.actual_hypergraph.remove_node(node, True)
            except MissingNodeError:
                self.actual_hypergraph.remove_node(int(node), True)
            except Exception as e:
                print(f"Errore durante la rimozione del nodo: {e}")
        self.cc = connected_components(self.actual_hypergraph)
        self.update_hypergraph()

    def remove_edges(self, edge_list: list) -> None:
        edge_list = list(set(edge_list))
        for edge in edge_list:
            try:
                if isinstance(self.actual_hypergraph, Hypergraph):
                    self.actual_hypergraph.remove_edge(edge)
                elif isinstance(self.actual_hypergraph, DirectedHypergraph):
                    self.actual_hypergraph.remove_edge(edge)
                elif isinstance(self.actual_hypergraph, TemporalHypergraph):
                    actual_edge = edge[0]
                    time = edge[1]
                    self.actual_hypergraph.remove_edge(actual_edge, time)
            except Exception:
                pass
        self.cc = connected_components(self.actual_hypergraph)
        self.update_hypergraph()

    def generate_random_hypergraph(self, values: dict) -> None:
        try:
            new_hypergraph = random_hypergraph(
                num_nodes=int(values["num_nodes"]),
                num_edges_by_size=values["edges_by_size"],
            )
            self.change_hypergraph(new_hypergraph)
        except Exception:
            raise

    def generate_random_uniform_graph(self, values: dict) -> None:
        try:
            new_hypergraph = random_uniform_hypergraph(
                int(values["num_nodes"]),
                int(values["edge_size"]),
                int(values["num_edges"]),
            )
            self.change_hypergraph(new_hypergraph)
        except Exception:
            raise

    def load_from_file(self, file_path: str) -> None:
        try:
            if file_path.endswith(".hif.json"):
                self.change_hypergraph(hypergraphx.readwrite.load_hif(file_path))
            else:
                self.change_hypergraph(hypergraphx.readwrite.load_hypergraph(file_path))
        except Exception:
            raise

    def save_to_file(self, file_path: str) -> None:
        try:
            if file_path.endswith(".hgx"):
                save_hypergraph(self.showed_hypergraph, file_path, binary=True)
            elif file_path.endswith(".json"):
                save_hypergraph(self.showed_hypergraph, file_path, binary=False)
            self.update_hypergraph()
        except Exception:
            raise

    def modify_edge_weight(self, edge, weight, time=None) -> None:
        match self.hypergraph_type:
            case HypergraphType.TEMPORAL:
                self.actual_hypergraph.set_weight(edge, time, weight)
            case _:
                self.actual_hypergraph.set_weight(edge, weight)
        self.update_hypergraph()

    def modify_edge_time(self, edge, old_time, time) -> None:
        weight = 0
        if self.actual_hypergraph.is_weighted():
            weight = self.actual_hypergraph.get_weight(edge, old_time)
        metadata = self.actual_hypergraph.get_edge_metadata(edge, old_time)
        self.actual_hypergraph.remove_edge(edge, old_time)
        if self.actual_hypergraph.is_weighted():
            self.actual_hypergraph.add_edge(edge, time, weight, metadata)
        else:
            self.actual_hypergraph.add_edge(edge, time, metadata=metadata)
        self.update_hypergraph()

    def update_selected_ccs(self, selected_cc_list: list) -> None:
        self.showed_hypergraph = Hypergraph()
        for cc in selected_cc_list:
            cc_hg: Hypergraph = self.actual_hypergraph.subhypergraph(cc)
            if self.actual_hypergraph.is_weighted():
                weights = cc_hg.get_weights()
                self.showed_hypergraph.add_edges(
                    cc_hg.get_edges(),
                    weights,
                    cc_hg.get_all_edges_metadata().values(),
                )
            else:
                self.showed_hypergraph.add_edges(
                    cc_hg.get_edges(),
                    metadata=cc_hg.get_all_edges_metadata().values(),
                )
        self.update_hypergraph(updated_ccs=True)

    def change_hypergraph(self, hypergraph) -> None:
        self.actual_hypergraph = hypergraph
        self.showed_hypergraph = hypergraph
        if isinstance(hypergraph, TemporalHypergraph):
            self.hypergraph_type = HypergraphType.TEMPORAL
        elif isinstance(hypergraph, DirectedHypergraph):
            self.hypergraph_type = HypergraphType.DIRECTED
        else:
            self.hypergraph_type = HypergraphType.NORMAL
            self.cc = connected_components(self.actual_hypergraph)
        self.update_hypergraph()

    def get_hypergraph(self):
        return self.showed_hypergraph

    # ------------------------------------------------------------------
    # Rendering
    # ------------------------------------------------------------------

    def plot_graph(self, input_values: dict) -> None:
        """
        Avvia il PlotWorker con i parametri forniti dalla vista.

        Parametri di ``input_values``
        ------------------------------
        curr_function : str
            Nome della funzione di disegno.
        community_model :
            Modello community già calcolato (usato se use_last=True).
        community_algorithm : str
            Nome dell'algoritmo di community detection.
        community_options_dict : dict
            Opzioni per il community detection.
        aggregation_options : dict
            Opzioni per la semplificazione/aggregazione.
        use_last : bool
            Se True riutilizza il community_model precedente.
        dictionary : dict
            Tutti i parametri grafici e di layout.
        """
        # Cancella eventuale thread in esecuzione
        if self.thread and self.thread.isRunning():
            self.thread.cancel()
            self.thread.quit()

        self.thread = PlotWorker(
            draw_function=input_values["curr_function"],
            hypergraph=self.showed_hypergraph,
            input_dictionary=input_values["dictionary"],
            model=input_values["community_model"],
            community_algorithm=input_values["community_algorithm"],
            community_options=input_values["community_options_dict"],
            use_last=input_values["use_last"],
            aggregation_options=input_values["aggregation_options"],
        )
        self.thread.progress.connect(self._on_drawing_finished)
        self.thread.start()

    def _on_drawing_finished(self, drawing_result) -> None:
        """
        Slot privato collegato al segnale ``progress`` del PlotWorker.
        Salva il risultato e lo rilancia verso la vista.
        """
        self.drawing_result = drawing_result
        self.finished_drawing.emit(drawing_result)

    # ------------------------------------------------------------------
    # Notifica aggiornamento ipergrafo
    # ------------------------------------------------------------------

    def update_hypergraph(self, updated_ccs: bool = False) -> None:
        self.updated_hypergraph.emit(
            {"hypergraph": self.showed_hypergraph, "updated_ccs": updated_ccs}
        )
