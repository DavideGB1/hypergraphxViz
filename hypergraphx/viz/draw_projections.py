...
from hypergraphx.viz.__support import (
    __filter_hypergraph,
...
def _draw_bipartite_on_ax(
    ...
    for node in node_list:
        if u is None:
            nx.draw_networkx_nodes(
                g, ax=ax, pos=pos, nodelist=[node], node_shape=graphicOptions.node_shape[node],
    ...
    for edge in edge_list:
        nx.draw_networkx_nodes(
            g, ax=ax, pos=pos, node_shape=graphicOptions.edge_shape[edge],
    ...
    if draw_labels:
        ...
        nx.draw_networkx_labels(
            g, ax=ax, pos=pos, labels=labels, font_size=graphicOptions.label_size,
    ...
    if hypergraph.is_weighted():
        ...
        nx.draw_networkx_labels(
            g, ax=ax, pos=pos_offsetted, labels=labels, font_size=graphicOptions.weight_size,
            font_color=graphicOptions.label_color
        )
    ...
    for edge in g.edges():
        ...
        nx.draw_networkx_edges(
            g, pos, edgelist=[edge], ax=ax, edge_color=color,
...
def draw_bipartite(
    ...
    for i, (time, hypergraph) in enumerate(hypergraphs.items()):
        ...
        data = _compute_bipartite_drawing_data(
            hypergraph, cardinality, x_heaviest, align, pos, u, k
        )
        _draw_bipartite_on_ax(current_ax, data, u, draw_labels, align, graphicOptions, **kwargs)
    ...
    return axs_flat if n_times == 1 else axs_flat[0]
...
def _draw_clique_on_ax(
    ...
    for edge in g.edges():
        nx.draw_networkx_edges(
            G=g, pos=pos, edgelist=[edge], ax=ax, edge_color=graphicOptions.edge_color[edge],
    ...
    for node in g.nodes():
        if u is None:
            nx.draw_networkx_nodes(
                g, pos, nodelist=[node], node_size=graphicOptions.node_size[node],
    ...
    if draw_labels:
        ...
        nx.draw_networkx_labels(
            G=g, pos=pos, ax=ax, labels=labels, font_size=graphicOptions.label_size,
...
def draw_clique(
    ...
    for i, (time, hypergraph) in enumerate(hypergraphs.items()):
        ...
        data = _compute_clique_drawing_data(
            hypergraph, cardinality, x_heaviest, iterations, pos, u, k, weight_positioning
        )
        _draw_clique_on_ax(current_ax, data, u, draw_labels, graphicOptions, **kwargs)
...
def _draw_extra_node_on_ax(
    ...
    if show_edge_nodes:
        ...
        for edge in edge_list:
            nx.draw_networkx_nodes(
                g, pos, nodelist=[edge], node_size=graphicOptions.node_size[edge],
        ...
        if is_weighted:
            labels = nx.get_node_attributes(g, 'weight')
            nx.draw_networkx_labels(
                g, ax=ax, pos=pos, labels=labels, font_size=graphicOptions.weight_size,
                font_color=graphicOptions.label_color, **kwargs
            )
...
def draw_extra_node(
    ...
    for i, (time, hypergraph) in enumerate(hypergraphs.items()):
        ...
        data = _compute_extra_node_drawing_data(
            hypergraph, cardinality, x_heaviest, ignore_binary_relations,
            weight_positioning, respect_planarity, iterations, pos, u, k, draw_edge_graph
        )
        _draw_extra_node_on_ax(
            current_ax, data, u, show_edge_nodes, draw_labels, graphicOptions, **kwargs
        )
